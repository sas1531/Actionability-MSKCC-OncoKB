...
action_levels_barplot_tx_fun <- function(cna_df, mut_df, fus_df, clin_df, data_freeze,
  ...
  prop_level_df_order <- prop_level_df %>%
    filter(HIGHEST_LEVEL %in% c("LEVEL_1_MSI-H_TMB-H", "LEVEL_1", "LEVEL_2", "LEVEL_3A", "LEVEL_3B", "LEVEL_4")) %>%
  ...
  # Save
  write.table(prop_level_df_order, "./actionability_levels_barplot_table_tx.txt", sep = "\t", row.names = F, quote = F)
  ...
  percent_bar_plot <- ggplot(prop_level_df_order, aes(y = freq, x = label, fill = HIGHEST_LEVEL)) +
    ...
    scale_fill_manual(values = c("#33A02C", "#1F78B4", "#984EA3", "#BE98CE", "#a8a8a8", "#ffdab9", "gray90"),
                      limits = c("LEVEL_1", "LEVEL_2", "LEVEL_3A", "LEVEL_3B", "LEVEL_4", "ONCOGENIC", "NO_LEVEL"),
                      labels = c("LEVEL 1", "LEVEL 2", "LEVEL 3A", "LEVEL 3B", "LEVEL 4", "ONCOGENIC", "NO LEVEL")) +
    scale_x_discrete(position = "top",
                     limits = cancer_order) +
    ylab("Frequency") +
    labs(fill = "Highest Level of Tx Evidence")
...
# Create actionability level barplot for Dx
action_levels_barplot_dx_fun <- function(cna_df, mut_df, fus_df, clin_df, data_freeze,
                                      status = c("somatic", "germline", "both"),
                                      group_col,
                                      consent_col,
                                      msi_tmb_status,
  ...
  if (msi_tmb_status == TRUE){
    ...
    msi_tmb_df <- msi_tmb_df %>%
      dplyr::select(SAMPLE_ID) %>% 
      mutate_if(is.factor, as.character) %>%
      mutate(HIGHEST_DX_LEVEL = "LEVEL_1_MSI-H_TMB-H") %>%
      distinct()
  } else {
    msi_tmb_df <- data.frame(SAMPLE_ID = character(), HIGHEST_DX_LEVEL = character())
  }
  ...
  # Add group column
  clin_df <- clin_df %>%
    dplyr::rename(HIGHEST_DX_LEVEL_FINAL = HIGHEST_DX_LEVEL) %>%
    mutate(SAMPLE_ID = as.character(SAMPLE_ID),
           HIGHEST_DX_LEVEL_FINAL = as.character(HIGHEST_DX_LEVEL_FINAL)) %>%
    filter(SAMPLE_ID %in% data_freeze$SAMPLE_ID) %>%
    left_join(data_freeze[,c("SAMPLE_ID", group_col, consent_col)], by = c("SAMPLE_ID")) %>%
    mutate(HIGHEST_DX_LEVEL_FINAL = ifelse(SAMPLE_ID %in% msi_tmb_df$SAMPLE_ID, "LEVEL_1_MSI-H_TMB-H", HIGHEST_DX_LEVEL_FINAL))
  group_col_dup <- paste0(group_col, ".y")
  colnames(clin_df)[which(names(clin_df) == group_col_dup)] <- group_col
  colnames(clin_df)[which(names(clin_df) == consent_col)] <- "consent"
  ...
  # Fix column names if upper
  # Filter for columns of interest
  col_list <- c("SAMPLE_ID", "LEVEL_Dx1", "LEVEL_Dx2", "LEVEL_Dx3", "HIGHEST_DX_LEVEL", "oncogenic")
  ...
  fus_df <- fus_df %>%
    dplyr::rename_all(recode, 
                      ...
                      ONCOGENIC = "oncogenic") %>%
    mutate(SAMPLE_ID = as.character(SAMPLE_ID),
           HIGHEST_DX_LEVEL = as.character(HIGHEST_DX_LEVEL)) %>%
    filter(SAMPLE_ID %in% data_freeze$SAMPLE_ID) %>%
    mutate(HIGHEST_DX_LEVEL = ifelse(SAMPLE_ID %in% msi_tmb_df$SAMPLE_ID, "LEVEL_1_MSI-H_TMB-H", HIGHEST_DX_LEVEL)) %>%
    dplyr::select(col_list)
  ...
  master_df <- rbind(master_df, mut_df)
  master_df <- master_df %>%
    dplyr::select(SAMPLE_ID, HIGHEST_DX_LEVEL) %>%
    filter(HIGHEST_DX_LEVEL != "") %>%
    mutate_if(is.factor, as.character) %>%
    group_by(SAMPLE_ID) %>%
    dplyr::arrange(HIGHEST_DX_LEVEL) %>%
    dplyr::slice(1) %>%
    ungroup()
  ...
  # Filter if germline
  if (status == "germline") {
    clin_df <- left_join(clin_df, mut_germ_df[,c("SAMPLE_ID", "HIGHEST_DX_LEVEL")])
    clin_df <- clin_df %>% mutate_if(is.factor, as.character)
    clin_df$HIGHEST_DX_LEVEL_FINAL <- ifelse(clin_df$SAMPLE_ID %in% mut_germ_df$SAMPLE_ID, clin_df$HIGHEST_DX_LEVEL, "NO_LEVEL")
    # Get list of sample with oncogenic alteration
    onco_samp_list <- mut_df$SAMPLE_ID
  } else if (status == "somatic") {
    clin_df <- left_join(clin_df, master_df, by = "SAMPLE_ID")
    clin_df <- clin_df %>% mutate_if(is.factor, as.character)
    clin_df$HIGHEST_DX_LEVEL_FINAL <- ifelse(clin_df$SAMPLE_ID %in% mut_germ_df$SAMPLE_ID,
                                             clin_df$HIGHEST_DX_LEVEL, clin_df$HIGHEST_DX_LEVEL_FINAL)
    # Merge to make master oncogenic list of samples
    all_df <- full_join(cna_df, fus_df, by = "SAMPLE_ID")
    all_df <- full_join(all_df, mut_df, by = "SAMPLE_ID")
    # Get list of sample with oncogenic alteration
  ...
  # Fill in the highest level blanks:
  clin_df$HIGHEST_DX_LEVEL_FINAL <- as.character(clin_df$HIGHEST_DX_LEVEL_FINAL)
  clin_df$HIGHEST_DX_LEVEL_FINAL[clin_df$HIGHEST_DX_LEVEL_FINAL == "" | is.na(clin_df$HIGHEST_DX_LEVEL_FINAL) == T] <- "NO_LEVEL"
  #clin_df$HIGHEST_DX_LEVEL_FINAL[(clin_df$SAMPLE_ID %in% onco_samp_list) & (clin_df$HIGHEST_DX_LEVEL_FINAL == "NO_LEVEL") ] <- "ONCOGENIC"
  
  # For highest level of actionability, calculate the percentage of each level by subtype
  prop_level_df <- freq_dataframe(clin_df, group_col, "HIGHEST_DX_LEVEL_FINAL")
  
  # Set level order
  level_order <- c("LEVEL_Dx1", "LEVEL_Dx2", "LEVEL_Dx3", "NO_LEVEL")
  prop_level_df$HIGHEST_DX_LEVEL_FINAL <- factor(prop_level_df$HIGHEST_DX_LEVEL_FINAL, levels = level_order)
  ...
  # # Arrange by frequency of actionability
  # prop_level_df <- prop_level_df %>%
  #   arrange(HIGHEST_DX_LEVEL_FINAL, desc(freq))
  
  # Arrange by frequency of combined top 4 levels of actionability
  prop_level_df_order <- prop_level_df %>%
    filter(HIGHEST_DX_LEVEL_FINAL %in% c("LEVEL_Dx1", "LEVEL_Dx2", "LEVEL_Dx3")) %>%
    group_by(CANCER_TYPE) %>%
    dplyr::mutate(sum_freq = sum(freq)) %>%
    right_join(prop_level_df) %>%
    dplyr::arrange(desc(sum_freq), HIGHEST_DX_LEVEL_FINAL, desc(freq)) %>%
    dplyr::rename(total_count = Freq) %>%
    mutate(CANCER_TYPE = factor(CANCER_TYPE, levels = unique(CANCER_TYPE)))
  
  # Save
  write.table(prop_level_df_order, "./actionability_levels_barplot_table_dx.txt", sep = "\t", row.names = F, quote = F)
  
  # Set orders
  cancer_order <- unique(prop_level_df_order$label)
  
  # Plot breakdown of levels of evidence as a percentage by sarcoma subtype
  percent_bar_plot <- ggplot(prop_level_df_order, aes(y = freq, x = label, fill = HIGHEST_DX_LEVEL_FINAL)) +
    geom_col(position = position_stack(reverse = TRUE)) +
    theme(axis.text.x = element_text(angle = 45, hjust = 0, size = 6),
          axis.text.y = element_text(size = 6),
          axis.ticks.x = element_blank(),
          ...
          legend.box.margin=margin(-10,0,-10,-5)) +
    scale_y_continuous(limits=c(0, 1.00), expand = c(0, 0)) +
    scale_fill_manual(values = c("#33A02C", "#1F78B4", "#984EA3", "gray90"),
                      limits = c("LEVEL_Dx1", "LEVEL_Dx2", "LEVEL_Dx3", "NO_LEVEL"),
                      labels = c("LEVEL Dx1", "LEVEL Dx2", "LEVEL Dx3", "NO LEVEL")) +
    scale_x_discrete(position = "top",
                     limits = cancer_order) +
    ylab("Frequency") +
    labs(fill = "Highest Level of Dx Evidence")
...
# Create actionability level barplot for Px
action_levels_barplot_px_fun <- function(cna_df, mut_df, fus_df, clin_df, data_freeze,
                                         status = c("somatic", "germline", "both"),
                                         group_col,
                                         consent_col,
                                         msi_tmb_status,
  ...
  if (msi_tmb_status == TRUE){
    ...
    msi_tmb_df <- msi_tmb_df %>%
      dplyr::select(SAMPLE_ID) %>% 
      mutate_if(is.factor, as.character) %>%
      mutate(HIGHEST_PX_LEVEL = "LEVEL_1_MSI-H_TMB-H") %>%
      distinct()
  } else {
    msi_tmb_df <- data.frame(SAMPLE_ID = character(), HIGHEST_PX_LEVEL = character())
  }
  ...
  # Add group column
  clin_df <- clin_df %>%
    dplyr::rename(HIGHEST_PX_LEVEL_FINAL = HIGHEST_PX_LEVEL) %>%
    mutate(SAMPLE_ID = as.character(SAMPLE_ID),
           HIGHEST_PX_LEVEL_FINAL = as.character(HIGHEST_PX_LEVEL_FINAL)) %>%
    filter(SAMPLE_ID %in% data_freeze$SAMPLE_ID) %>%
    left_join(data_freeze[,c("SAMPLE_ID", group_col, consent_col)], by = c("SAMPLE_ID")) %>%
    mutate(HIGHEST_PX_LEVEL_FINAL = ifelse(SAMPLE_ID %in% msi_tmb_df$SAMPLE_ID, "LEVEL_1_MSI-H_TMB-H", HIGHEST_PX_LEVEL_FINAL))
  group_col_dup <- paste0(group_col, ".y")
  colnames(clin_df)[which(names(clin_df) == group_col_dup)] <- group_col
  colnames(clin_df)[which(names(clin_df) == consent_col)] <- "consent"
  ...
  # Fix column names if upper
  # Filter for columns of interest
  col_list <- c("SAMPLE_ID", "LEVEL_Px1", "LEVEL_Px2", "LEVEL_Px3", "HIGHEST_PX_LEVEL", "oncogenic")
  ...
  fus_df <- fus_df %>%
    dplyr::rename_all(recode, 
                      ...
                      ONCOGENIC = "oncogenic") %>%
    mutate(SAMPLE_ID = as.character(SAMPLE_ID),
           HIGHEST_PX_LEVEL = as.character(HIGHEST_PX_LEVEL)) %>%
    filter(SAMPLE_ID %in% data_freeze$SAMPLE_ID) %>%
    mutate(HIGHEST_PX_LEVEL = ifelse(SAMPLE_ID %in% msi_tmb_df$SAMPLE_ID, "LEVEL_1_MSI-H_TMB-H", HIGHEST_PX_LEVEL)) %>%
    dplyr::select(col_list)
  ...
  master_df <- rbind(master_df, mut_df)
  master_df <- master_df %>%
    dplyr::select(SAMPLE_ID, HIGHEST_PX_LEVEL) %>%
    filter(HIGHEST_PX_LEVEL != "") %>%
    mutate_if(is.factor, as.character) %>%
    group_by(SAMPLE_ID) %>%
    dplyr::arrange(HIGHEST_PX_LEVEL) %>%
    dplyr::slice(1) %>%
    ungroup()
  ...
  # Filter if germline
  if (status == "germline") {
    clin_df <- left_join(clin_df, mut_germ_df[,c("SAMPLE_ID", "HIGHEST_PX_LEVEL")])
    clin_df <- clin_df %>% mutate_if(is.factor, as.character)
    clin_df$HIGHEST_PX_LEVEL_FINAL <- ifelse(clin_df$SAMPLE_ID %in% mut_germ_df$SAMPLE_ID, clin_df$HIGHEST_PX_LEVEL, "NO_LEVEL")
    # Get list of sample with oncogenic alteration
    onco_samp_list <- mut_df$SAMPLE_ID
  } else if (status == "somatic") {
    clin_df <- left_join(clin_df, master_df, by = "SAMPLE_ID")
    clin_df <- clin_df %>% mutate_if(is.factor, as.character)
    clin_df$HIGHEST_PX_LEVEL_FINAL <- ifelse(clin_df$SAMPLE_ID %in% mut_germ_df$SAMPLE_ID,
                                             clin_df$HIGHEST_PX_LEVEL, clin_df$HIGHEST_PX_LEVEL_FINAL)
    # Merge to make master oncogenic list of samples
    all_df <- full_join(cna_df, fus_df, by = "SAMPLE_ID")
    all_df <- full_join(all_df, mut_df, by = "SAMPLE_ID")
    # Get list of sample with oncogenic alteration
  ...
  # Fill in the highest level blanks:
  clin_df$HIGHEST_PX_LEVEL_FINAL <- as.character(clin_df$HIGHEST_PX_LEVEL_FINAL)
  clin_df$HIGHEST_PX_LEVEL_FINAL[clin_df$HIGHEST_PX_LEVEL_FINAL == "" | is.na(clin_df$HIGHEST_PX_LEVEL_FINAL) == T] <- "NO_LEVEL"
  #clin_df$HIGHEST_PX_LEVEL_FINAL[(clin_df$SAMPLE_ID %in% onco_samp_list) & (clin_df$HIGHEST_PX_LEVEL_FINAL == "NO_LEVEL") ] <- "ONCOGENIC"
  
  # For highest level of actionability, calculate the percentage of each level by subtype
  prop_level_df <- freq_dataframe(clin_df, group_col, "HIGHEST_PX_LEVEL_FINAL")
  
  # Set level order
  level_order <- c("LEVEL_Px1", "LEVEL_Px2", "LEVEL_Px3", "NO_LEVEL")
  prop_level_df$HIGHEST_PX_LEVEL_FINAL <- factor(prop_level_df$HIGHEST_PX_LEVEL_FINAL, levels = level_order)
  ...
  # # Arrange by frequency of actionability
  # prop_level_df <- prop_level_df %>%
  #   arrange(HIGHEST_PX_LEVEL_FINAL, desc(freq))
  
  # Arrange by frequency of combined top 4 levels of actionability
  prop_level_df_order <- prop_level_df %>%
    filter(HIGHEST_PX_LEVEL_FINAL %in% c("LEVEL_Px1", "LEVEL_Px2", "LEVEL_Px3")) %>%
    group_by(CANCER_TYPE) %>%
    dplyr::mutate(sum_freq = sum(freq)) %>%
    right_join(prop_level_df) %>%
    dplyr::arrange(desc(sum_freq), HIGHEST_PX_LEVEL_FINAL, desc(freq)) %>%
    dplyr::rename(total_count = Freq) %>%
    mutate(CANCER_TYPE = factor(CANCER_TYPE, levels = unique(CANCER_TYPE)))
  
  # Save
  write.table(prop_level_df_order, "./actionability_levels_barplot_table_px.txt", sep = "\t", row.names = F, quote = F)
  
  # Set orders
  cancer_order <- unique(prop_level_df_order$label)
  
  # Plot breakdown of levels of evidence as a percentage by sarcoma subtype
  percent_bar_plot <- ggplot(prop_level_df_order, aes(y = freq, x = label, fill = HIGHEST_PX_LEVEL_FINAL)) +
    geom_col(position = position_stack(reverse = TRUE)) +
    theme(axis.text.x = element_text(angle = 45, hjust = 0, size = 6),
          axis.text.y = element_text(size = 6),
          axis.ticks.x = element_blank(),
    ...
    scale_y_continuous(limits=c(0, 1.00), expand = c(0, 0)) +
    scale_fill_manual(values = c("#33A02C", "#1F78B4", "#984EA3", "gray90"),
                      limits = c("LEVEL_Px1", "LEVEL_Px2", "LEVEL_Px3", "NO_LEVEL"),
                      labels = c("LEVEL Px1", "LEVEL Px2", "LEVEL Px3", "NO LEVEL")) +
    scale_x_discrete(position = "top",
                     limits = cancer_order) +
    ylab("Frequency") +
    labs(fill = "Highest Level of Px Evidence")
...
action_alterations_barplot_tx_fun <- function(cna_df, mut_df, fus_df, clin_df, data_freeze,
                                           status = c("somatic", "germline", "both"),
  ...
  # Save
  write.table(alt_final, "actionability_master_alterations_table_tx.txt", sep = "\t", row.names = F, quote = F)
  ...
  # Save
  write.table(alt_final, "actionability_master_alterations_highest_level_table_tx.txt", sep = "\t", row.names = F, quote = F)
  ...
  # Save
  write.table(prop_alteration_df, "actionability_alterations_barplot_table_tx.txt", sep = "\t", row.names = F, quote = F)
...
# Create actionability alteration barplot
action_alterations_barplot_dx_fun <- function(cna_df, mut_df, fus_df, clin_df, data_freeze,
                                              status = c("somatic", "germline", "both"),
                                              group_col, consent_col,
                                              prop_level_df = "./actionability_levels_barplot_table_dx.txt",
                                              only_highest_level = F,
                                              msi_tmb_status,
                                              msi_tmb_df){
  # Read in data
  ...
  # Save
  write.table(alt_final, "actionability_master_alterations_table_dx.txt", sep = "\t", row.names = F, quote = F)
  
  ########## optional select only the highest level ##########
  
  if (only_highest_level == T){
    alt_final <- alt_final %>% 
      left_join(dplyr::select(clin_df, SAMPLE_ID, HIGHEST_DX_LEVEL), by = "SAMPLE_ID") %>%
      mutate_if(is.factor, as.character) %>%
  ...
  # Save
  write.table(alt_final, "actionability_master_alterations_highest_level_table_dx.txt", sep = "\t", row.names = F, quote = F)
  ...
  # Save
  write.table(prop_alteration_df, "actionability_alterations_barplot_table_dx.txt", sep = "\t", row.names = F, quote = F)
...
# Create actionability alteration barplot
action_alterations_barplot_px_fun <- function(cna_df, mut_df, fus_df, clin_df, data_freeze,
                                              status = c("somatic", "germline", "both"),
                                              group_col, consent_col,
  ...
  # Save
  write.table(alt_final, "actionability_master_alterations_table_px.txt", sep = "\t", row.names = F, quote = F)
  
  ########## optional select only the highest level ##########
  
  if (only_highest_level == T){
    alt_final <- alt_final %>% 
      left_join(dplyr::select(clin_df, SAMPLE_ID, HIGHEST_PX_LEVEL), by = "SAMPLE_ID") %>%
      mutate_if(is.factor, as.character) %>%
  ...
  # Save
  write.table(alt_final, "actionability_master_alterations_highest_level_table_px.txt", sep = "\t", row.names = F, quote = F)
  ...
  # Save
  write.table(prop_alteration_df, "actionability_alterations_barplot_table_px.txt", sep = "\t", row.names = F, quote = F)
...
action_main_tx_fun <- function(cna_df, mut_df, fus_df, clin_df, data_freeze,
  ...
  write.table(prop_main_plot_df, "actionability_main_plot_data_tx.txt", sep = "\t", quote = F, row.names = F)
...
# Create actionability alterations main plot
action_main_dx_fun <- function(cna_df, mut_df, fus_df, clin_df, data_freeze,
  ...
  mut_df <- mut_df %>%
    inner_join(dplyr::select(data_freeze, SAMPLE_ID, cancer_type), by = "SAMPLE_ID") %>%
    dplyr::select(SAMPLE_ID, Hugo_Symbol, Variant_Type, LEVEL_Dx1, LEVEL_Dx2, LEVEL_Dx3,
                  Highest_level, cancer_type, HGVSp_Short, Mutation_Status)
  mut_df <- melt(mut_df, id.vars = c("SAMPLE_ID", "Hugo_Symbol", "Variant_Type", "Highest_level",
  ...
  # Write out data frame
  write.table(prop_main_plot_df, "actionability_main_plot_data_dx.txt", sep = "\t", quote = F, row.names = F)
...
# Create actionability alterations main plot
action_main_px_fun <- function(cna_df, mut_df, fus_df, clin_df, data_freeze,
                               path_df, tsg_list, fusion_list,
                               prop_level_df = "./actionability_levels_barplot_table_px.txt",
                               group_col,
                               consent_col,
  ...
  mut_df <- mut_df %>%
    inner_join(dplyr::select(data_freeze, SAMPLE_ID, cancer_type), by = "SAMPLE_ID") %>%
    dplyr::select(SAMPLE_ID, Hugo_Symbol, Variant_Type, LEVEL_Px1, LEVEL_Px2, LEVEL_Px3,
                  Highest_level, cancer_type, HGVSp_Short, Mutation_Status)
  mut_df <- melt(mut_df, id.vars = c("SAMPLE_ID", "Hugo_Symbol", "Variant_Type", "Highest_level",
                                     "cancer_type", "HGVSp_Short", "Mutation_Status"))
  ...
  # Write out data frame
  write.table(prop_main_plot_df, "actionability_main_plot_data_px.txt", sep = "\t", quote = F, row.names = F)
...
#------------------------
# Original scripts for therapeutic data
